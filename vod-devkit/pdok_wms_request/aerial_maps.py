...
class AerialMapRetriever:
    def __init__(self,
        ...
        # PDOK WMS는 픽셀 수가 크면 XML(ServiceException: image size too large)을 반환함.
        # (기존 4000 근처) → 2048 이하로 맞추면 Actueel_orthoHR 등에서 정상 응답(이미지).
        self.max_width = 2048
    ...
    def get_map_from_corners(self, bbox, x_pixels=None, y_pixels=None, resolution=None):
        ...
        if "xml" in ctype or response.content.lstrip().startswith(b"<?xml"):
            raise ValueError(
                "WMS가 이미지 대신 XML을 반환했습니다. "
                "보통 'image size too large'이며, resolution을 올리거나(예: 0.25) bbox/픽셀을 줄이세요."
                f"\n{response.text[:500]}"
            )
