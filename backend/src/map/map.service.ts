...
export class MapService {
  ...
  private async buildSyntheticRadarSnapshot(): Promise<RadarSnapshotDto> {
    ...
    const methodology = {
      ...
      poseAndDistanceNote:
        '거리(range)는 FMCW 채널에서 왕복 지연(비트 주파수와 대응)으로 추정하고, 방위·고도는 배열/빔스캔에 따른 각 스펙트럼에서 피크를 고릅니다. 도플러는 레이더–표적 연선 방향의 상대속도(접근/이탈)에 비례합니다. “어디를 보고 있는지”는 본 데모에서 레이더 주시 방위(빔 중심)로 표시하고, 기준 표적 쪽을 향하도록 맞춥니다. 차량의 주행 방향은 지도상 OSRM 도로 궤적·핀 이동 방향으로 가시화할 수 있으며, 완전한 자세 추정은 카메라·IMU·상태 추정 필터와 결합하는 것이 일반적입니다.',
      preprocessingNote:
      ...
      trainingNote:
        '공개 데이터(View-of-Delft 등)에서 RD 맵 또는 3+1D 포인트와 라벨(거리·방위·도플러/반사 강도)을 쌍으로 구성합니다. RD 기반이면 U-Net/ResNet류로 피크·경계 회귀, 포인트 기반이면 PointNet/센서 특화 네트워크로 분할·회귀를 학습하고, 손실은 거리·각·도플러 오차에 프레임 간 연속성 보조 항을 더하는 식으로 설계할 수 있습니다.',
      demoImplementationNote:
