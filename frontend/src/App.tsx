...
import {
  ...
  ORBITAL_SLOT_BY_ID,
  ORBITAL_SAR_NAMES,
  buildOrbitalFootprintsGeoJson,
  buildOrbitalTracesGeoJson,
  isOrbitalSarAssetId,
  orbitalGroundPosition,
  type SarObservationMode,
} from './battlefield/orbitalSarMock'
import { resolveOpsRegionSearch } from './battlefield/opsRegionSearchCatalog'
import {
...
import {
  ...
  snapEnemyMbtPoseToLandCorridor,
  snapMarchPolylineVertices,
  type MarchPoint,
...
import {
  ...
  BATTALION_SCENARIO,
  DMZ_MDL_LATITUDE_FALLBACK_GEOJSON,
  DMZ_PARALLEL_38_N,
  MDL_OSM_GEOJSON_PUBLIC_PATH,
...
type ScenarioEntity = {
  ...
  /** GRD 위험도 점수(더미) */
  grdRiskScore?: number
  /** 적 단대호(에셜론) — 지도 심볼 상단 부호용 */
  echelonLevel?: UnitLevel
}
...
type SelectedObjectDetail = {
  ...
  /** 위성 SAR 상세 — 모드는 `orbitalSarModes` 상태와 연동 */
  orbitalSarAssetId?: number
  /** LEO 궤도고(km) — 패널 표시용 */
  orbitAltitudeKm?: number
  /** 지상 투영 궤적 속도(km/h) */
  orbitalGroundSpeedKmh?: number
}
...
const SERVICE_CATEGORY_LABEL: Record<ServiceAssetCategory, string> = {
  ...
  /** 목록 순서: 지상 레이더 다음(자산 현황 패널) */
  SATELLITE_SAR: '위성 SAR',
}
...
const SERVICE_CATEGORY_COLOR: Record<ServiceAssetCategory, string> = {
  ...
  DRONE: '#c026d3',
  GROUND_RADAR: '#f59e0b',
  SATELLITE_SAR: '#06b6d4',
}

/** 우측 패널 「자산 현황」— DB·지도 센서 자산만 */
const SERVICE_ASSET_STATUS_CATEGORIES: readonly ServiceAssetCategory[] = [
  'GROUND_RADAR',
  'SATELLITE_SAR',
  'UAV',
  'DRONE',
...
/** 우측 패널 「아군 현황」— 전투·지휘 부대만 */
const SERVICE_FRIENDLY_FORCE_CATEGORIES: readonly ServiceAssetCategory[] = [
  'DIVISION',
  'UPPER_COMMAND',
  'ARTILLERY',
  'ARMOR',
...
const SERVICE_ASSET_STATUS_SIDEBAR_LABEL: Partial<Record<ServiceAssetCategory, string>> = {
  GROUND_RADAR: '지상감시 레이더',
  SATELLITE_SAR: '위성 SAR',
  UAV: 'UAV(EO/IR)',
  DRONE: '드론',
...
const SERVICE_FRIENDLY_FORCE_SIDEBAR_LABEL: Partial<Record<ServiceAssetCategory, string>> = {
  DIVISION: '사단',
  UPPER_COMMAND: '상급지휘소',
  ARTILLERY: '포병부대',
  ARMOR: '전차부대',
...
const SERVICE_ASSET_ROSTER_ORDER: readonly ServiceAssetCategory[] = [
  'GROUND_RADAR',
  'SATELLITE_SAR',
  'UAV',
  'DRONE',
...
const FRIENDLY_ROSTER_CATEGORY_SET = new Set<ServiceAssetCategory>(SERVICE_FRIENDLY_FORCE_CATEGORIES)
...
const SCENARIO_DB_ENEMY_ENTITY_IDS = [9001, 9002, 9003, 9004, 9005, 9006, 9007, 9008, 9009, 9010] as const

const ENEMY_INGRESS_LAND_GUARD = {
  westLng: 126.05,
  eastLng: 128.35,
  southLat: 37.2,
  northLat: 40.8,
...
function normalizeEnemyIngressPoint(lat: number, lng: number): { lat: number; lng: number } {
  const boxed = {
    lat: Math.min(ENEMY_INGRESS_LAND_GUARD.northLat, Math.max(ENEMY_INGRESS_LAND_GUARD.southLat, lat)),
    lng: Math.min(ENEMY_INGRESS_LAND_GUARD.eastLng, Math.max(ENEMY_INGRESS_LAND_GUARD.westLng, lng)),
  }
...
const ORBITAL_LEO_ALTITUDE_KM = 650
const ORBITAL_GROUND_SPEED_KMH = 27500

function buildOrbitalSarServiceAssets(): ServiceAssetPoint[] {
  return ORBITAL_SAR_IDS.map((id, slot) => {
    const pos = orbitalGroundPosition(slot, 0)
    return {
      id,
      name: ORBITAL_SAR_NAMES[slot]!,
      lat: pos.lat,
      lng: pos.lng,
      unitCode: buildUnitIdentificationCode('SATELLITE_SAR', id),
      category: 'SATELLITE_SAR',
      level: '대대',
      formation: 'LEO 궤도·SAR 페이로드',
      elevationM: Math.round(ORBITAL_LEO_ALTITUDE_KM * 1000),
      mgrs: latLngToMgrsSafe(pos.lat, pos.lng),
      readiness: '최고',
      mission: `LEO SAR · 궤도고 약 ${ORBITAL_LEO_ALTITUDE_KM} km · 지상 투영 속도 약 ${(ORBITAL_GROUND_SPEED_KMH / 1000).toFixed(0)}k km/h`,
...
function scenarioEntityToGeoJsonProperties(entity: ScenarioEntity): {
  ...
  const isEnemyMbt = entity.relation === 'ENEMY' && entity.kind === 'MBT'
  return {
    id: entity.id,
    ...
    ...(isEnemyMbt
      ? { echelonRankMark: scenarioEnemyEchelonMark(entity), enemyEchelonLevel: scenarioEnemyEchelonLevel(entity) }
      : {}),
  }
}
...
/** 아군 자산 MapLibre 심볼 위에 단대호 에셜론 부호(텍스트) */
const SERVICE_ASSETS_ECHELON_LAYER_ID = 'service-assets-echelon-layer'
...
const SERVICE_SCENARIO_ENEMY_SYMBOL_IMAGE_ID = 'service-scenario-enemy-hostile-diamond-v1'
const SERVICE_SCENARIO_ENEMY_SYMBOL_LAYER_ID = 'service-scenario-enemy-symbol-layer'
/** 적 MBT 심볼 위 단대호(에셜론) 텍스트 */
const SERVICE_SCENARIO_ENEMY_ECHELON_LAYER_ID = 'service-scenario-enemy-echelon-layer'
...
const SERVICE_MDL_LINE_SOURCE_ID = 'service-mdl-line-source'
const SERVICE_MDL_LINE_LAYER_ID = 'service-mdl-line-layer'
...
const SERVICE_ORBITAL_SAR_TRACE_SOURCE_ID = 'service-orbital-sar-trace-source'
const SERVICE_ORBITAL_SAR_TRACE_LAYER_ID = 'service-orbital-sar-trace-layer'
...
/** 드론 출동 추천 모달 — 표적 이미지(EO/IR·ATR K2 식별 데모) */
...
const SERVICE_ASSET_SYMBOL_IMAGE_ID: Record<ServiceAssetCategory, string> = {
  ...
  SATELLITE_SAR: 'service-asset-symbol-satellite-sar',
}
...
/** 드론·UAV 자산 심볼·원 반경을 줄여 이동 표시와 겹칠 때 과도하게 크게 보이지 않게 함 */
const SERVICE_ASSETS_DRONE_CIRCLE_RADIUS = 5.2
...
const SERVICE_ASSETS_SYMBOL_ICON_SIZE_LAYOUT: maplibregl.ExpressionSpecification = [
  ...
  ['get', 'category'],
  'DRONE',
  SERVICE_ASSETS_DRONE_SYMBOL_ICON_SIZE,
  'UAV',
...
/** 적 기갑 — 적(적대) 마름모 프레임 + 타원(전장) 글리프 (단대호 에셜론은 MapLibre 텍스트 레이어) */
function buildScenarioEnemyArmoredSymbolImage(): ImageData | null {
  ...
  const cx = size / 2
  const cy = size / 2 + 0.5
  const halfW = (size - 16) / 2
  const halfH = (size - 22) / 2
  ...
  ctx.fillStyle = 'rgba(254, 231, 199, 0.94)'
  ctx.fill()

  ctx.strokeStyle = 'rgba(146, 64, 14, 0.98)'
  ctx.lineWidth = 2.75
  ctx.lineJoin = 'round'
  ctx.stroke()
  ...
  ctx.lineCap = 'round'
  ctx.beginPath()
  ctx.ellipse(cx, cy + 0.5, 10.5, 6.1, 0, 0, Math.PI * 2)
  ctx.stroke()
...
const SERVICE_MAP_OBJECT_CLICK_LAYER_IDS: string[] = [
  ...
  SERVICE_ASSETS_ECHELON_LAYER_ID,
  ...
  SERVICE_SCENARIO_ENEMY_ECHELON_LAYER_ID,
  ...
  SERVICE_ORBITAL_SAR_FOOTPRINT_FILL_LAYER_ID,
  SERVICE_ORBITAL_SAR_FOOTPRINT_LINE_LAYER_ID,
  SERVICE_ORBITAL_SAR_TRACE_LAYER_ID,
...
type UnitLevel = '소대' | '중대' | '대대' | '연대' | '사단' | '군단' | '특수임무부대'
...
type UavDispatchTarget = {
  ...
  enemyId?: number | null
  enemyName?: string
  /** GRD·파란(일반차량) 클릭 시 모달을 SAR 탐지 이미지 탭으로 연다 */
  grdModalEntry?: 'sar_detection'
}
...
type EnemyInfiltration = {
  ...
  lat: number
  lng: number
  /** 해발 표고(m) — `/map/infiltrations` DB 필드 */
  elevationM?: number | null
 ...
 * - 특수임무부대(전투단): ◆
 */
const ECHELON_RANK_MARK: Record<UnitLevel, string> = {
  소대: '•',
  중대: '|',
  대대: '||',
  연대: '|||',
...
const ECHELON_RANK_KIND: Record<UnitLevel, 'dot' | 'bar' | 'cross' | 'special'> = {
  소대: 'dot',
  중대: 'bar',
  대대: 'bar',
  연대: 'bar',
...
function echelonRankMarkFromLevel(level: string | undefined | null): string {
  if (!level) return ''
  const mark = ECHELON_RANK_MARK[level as UnitLevel]
  return mark ?? ''
}

function scenarioEnemyEchelonLevel(entity: ScenarioEntity): UnitLevel {
  if (entity.echelonLevel) return entity.echelonLevel
  if (entity.enemyCategory === 'grd_motion_cluster') return '중대'
  if (entity.enemyCategory === 'mechanized_battalion') return '연대'
  return '대대'
}

function scenarioEnemyEchelonMark(entity: ScenarioEntity): string {
  return echelonRankMarkFromLevel(scenarioEnemyEchelonLevel(entity))
}
...
function buildFriendlyTacticalPinHTML(unit: FriendlyUnit): string {
  ...
  const rankMark = ECHELON_RANK_MARK[unit.level] ?? ''
  const rankKind = ECHELON_RANK_KIND[unit.level] ?? 'bar'
  return `
    <div class="tactical-marker tactical-marker--friendly tactical-marker--sym-${sym} tactical-marker--loc-${loc} tactical-marker--str-${str}">
      ...
      <div class="tactical-marker__rank tactical-marker__rank--${rankKind}" aria-label="${unit.level}" title="${unit.level}">${rankMark}</div>
    ...
    if (tacticalAisUiEnabled) {
      ...
      infoContent.innerHTML = insetMinimal
        ? `
            ...
            ${
              enemy.elevationM != null && Number.isFinite(enemy.elevationM)
                ? `<p class="map-hover-minimal-line muted">표고 약 ${Math.round(enemy.elevationM)} m</p>`
                : ''
            }
          `
        : `
            ...
            ${
              enemy.elevationM != null && Number.isFinite(enemy.elevationM)
                ? `<p><strong>표고:</strong> ${Math.round(enemy.elevationM)} m (해발)</p>`
                : ''
            }
...
function HomePage({ user }: HomePageProps) {
  ...
  /** 카카오맵을 쓰지 않는 단독 모드에서만 MDL 근접·FMCW 락 상태 동기화(비교 모드는 왼쪽 지도가 담당) */
  useEffect(() => {
  ...
  const unitCounts = useMemo(() => {
    return friendlyUnits.reduce<Record<UnitLevel, number>>(
      ...
      { 소대: 0, 중대: 0, 대대: 0, 연대: 0, 사단: 0, 군단: 0, 특수임무부대: 0 },
    )
  }, [friendlyUnits])
  ...
  useEffect(() => {
    ...
    const onLoadKakaoMap = () => {
      ...
      kakaoMaps.load(() => {
        ...
        const drawMdlStraightFallback = () => {
          const mdlMainWest = new kakaoMaps.LatLng(DMZ_PARALLEL_38_N, ob.sw.lng)
          const mdlMainEast = new kakaoMaps.LatLng(DMZ_PARALLEL_38_N, ob.ne.lng)
          const mdlMainLine = new kakaoMaps.Polyline({
            path: [mdlMainWest, mdlMainEast],
            strokeWeight: 3,
            strokeColor: '#c2410c',
            strokeOpacity: 0.9,
            strokeStyle: 'dash',
          ...
          mdlMainLine.setMap(map)
          radarDisposables.push(mdlMainLine)
          const mdlInsetWest = new kakaoMaps.LatLng(DMZ_PARALLEL_38_N, ib.sw.lng)
          const mdlInsetEast = new kakaoMaps.LatLng(DMZ_PARALLEL_38_N, ib.ne.lng)
          const mdlInsetLine = new kakaoMaps.Polyline({
            path: [mdlInsetWest, mdlInsetEast],
            strokeWeight: 2,
            strokeColor: '#c2410c',
            strokeOpacity: 0.88,
            strokeStyle: 'dash',
          ...
          mdlInsetLine.setMap(insetMap)
        }

        const mdlLabelEl = document.createElement('div')
        mdlLabelEl.className = 'map-overview-region-label map-overview-region-label--mdl'
        mdlLabelEl.innerHTML =
          '<span class="map-overview-region-label__title">군사분계선(MDL)</span>' +
          '<span class="map-overview-region-label__sub">OpenStreetMap 곡선 불러오는 중… · ' +
          `<a class="map-mdl-ref" href="${MDL_REFERENCE_ARTICLE_URL}" target="_blank" rel="noopener noreferrer">참고</a></span>`
        const mdlLabelOv = new kakaoMaps.CustomOverlay({
          position: new kakaoMaps.LatLng(DMZ_PARALLEL_38_N + 0.07, (ob.sw.lng + ob.ne.lng) / 2),
          yAnchor: 0.5,
          xAnchor: 0.5,
          content: mdlLabelEl,
          zIndex: 10,
        })
        mdlLabelOv.setMap(map)
        radarDisposables.push(mdlLabelOv)

        void (async () => {
          try {
            const res = await fetch(MDL_OSM_GEOJSON_PUBLIC_PATH)
            ...
            if (!geom || geom.type !== 'MultiLineString' || !Array.isArray(geom.coordinates)) {
              throw new Error('invalid MDL GeoJSON')
            }
            let sumLat = 0
            let sumLng = 0
            let n = 0
            for (const line of geom.coordinates) {
              ...
              const path = line.map(([lng, lat]) => new kakaoMaps.LatLng(lat, lng))
              const plMain = new kakaoMaps.Polyline({
                path,
                strokeWeight: 2,
                strokeColor: '#c2410c',
                strokeOpacity: 0.9,
              ...
              plMain.setMap(map)
              radarDisposables.push(plMain)
              const plInset = new kakaoMaps.Polyline({
                path,
                strokeWeight: 2,
                strokeColor: '#c2410c',
                strokeOpacity: 0.85,
            ...
            if (n > 0) {
              mdlLabelOv.setPosition(new kakaoMaps.LatLng(sumLat / n + 0.035, sumLng / n))
            }
            mdlLabelEl.innerHTML =
              '<span class="map-overview-region-label__title">군사분계선(MDL)</span>' +
              '<span class="map-overview-region-label__sub">OpenStreetMap 관계 13563116 곡선(ODbL) · ' +
              `<a class="map-mdl-ref" href="${MDL_REFERENCE_ARTICLE_URL}" target="_blank" rel="noopener noreferrer">참고</a></span>`
          } catch {
            drawMdlStraightFallback()
            mdlLabelOv.setPosition(new kakaoMaps.LatLng(DMZ_PARALLEL_38_N + 0.07, (ob.sw.lng + ob.ne.lng) / 2))
            mdlLabelEl.innerHTML =
              '<span class="map-overview-region-label__title">군사분계선(MDL)</span>' +
              '<span class="map-overview-region-label__sub">OSM 곡선 로드 실패 · 북위38° 직선 근사 · ' +
              `<a class="map-mdl-ref" href="${MDL_REFERENCE_ARTICLE_URL}" target="_blank" rel="noopener noreferrer">참고</a></span>`
          }
        })()
      ...
      <div className="map-section">
        ...
        {enemyNearDmz38 && (
          <div className="scenario-dmz-alert" role="status">
            <span className="scenario-dmz-alert__badge">MDL</span>
            <span>
              <strong>군사분계선(MDL) 인접</strong> — 지도 주황 점선은 북위38° 근사.
            </span>
          </div>
        )}
        ...
        {tacticalPhaseUi === 'schematic' && schematicBounds && tacticalSimPoints && (
          <div className="tactical-phase-pane tactical-phase-pane--schematic">
            ...
            <p className="muted map-subtitle-hint">
              위·경도만 직교 투영한 선화면입니다. 군사분계선(MDL·근사)·C2·적·거리선·FMCW 부채꼴만 남기고 위성
              타일·부대 핀·SAR 원은 제거했습니다.
            </p>
        ...
        <div className="map-legend map-legend-tactical">
          ...
          <ul className="map-legend-list">
            ...
            <li>
              하단 글자: <strong>소·중·대·연·사·군·특</strong> = 소대/중대/대대/연대/사단/군단/특수임무부대,
              적은 <strong>적</strong> 표기.
            </li>
            <li>
...
function grdDispatchTitleByMotionId(motionId: string): string {
  return motionId === 'grd-mot-5' ? '전차 움직임 감지' : '이동감지'
}

function grdDispatchSummaryByMeta(
  motionId: string,
  meta: { classLabel: string; probPercent: number },
): string {
  const base = `${meta.classLabel} 의심 · 신뢰도 ${meta.probPercent}%`
  if (motionId === 'grd-mot-5') {
    return `${base} · GMTI 알고리즘 기반 이동체 변화 검출`
  }
  return base
}

/** 노란(전차)이 아닌 GRD 슬롯 — 지도·모달에서 파란 일반차량 구역으로 처리 */
function isGrdBlueVehicleClass(classLabel: string): boolean {
  return classLabel !== '전차'
}
...
function BattlefieldServicePage() {
  ...
  const [orbitalSarModes, setOrbitalSarModes] = useState<Record<number, SarObservationMode>>(() =>
    Object.fromEntries(ORBITAL_SAR_IDS.map((id) => [id, 'SPOTLIGHT' as const])) as Record<
      number,
      SarObservationMode
    >,
  )
  const [friendlyUnitsRaw, setFriendlyUnitsRaw] = useState<FriendlyUnit[]>([])
  const [enemyInfiltrationsDb, setEnemyInfiltrationsDb] = useState<EnemyInfiltration[]>([])
  ...
  /** 노란 GRD(전차) 링 중심: 적 MBT 최초 스폰 좌표에 한 번만 맞춘 뒤 고정(재개·남하 후에도 이동 안 함) */
  const [frozenYellowGrdMbtCentersByMotionId, setFrozenYellowGrdMbtCentersByMotionId] = useState<
    Record<string, { lat: number; lng: number }> | null
  >(null)

  const scenarioEntitiesResolved = useMemo(() => {
    const nonEnemy = DUMMY_SCENARIO_ENTITIES.filter((e) => e.relation !== 'ENEMY')
    const grdCluster = DUMMY_SCENARIO_ENTITIES.find((e) => e.id === 9050 && e.relation === 'ENEMY') ?? null
    const sortedEnemyRows = [...enemyInfiltrationsDb]
      .sort((a, b) => {
        const threatOrder: Record<EnemyInfiltration['threatLevel'], number> = { 높음: 0, 중간: 1, 낮음: 2 }
        return threatOrder[a.threatLevel] - threatOrder[b.threatLevel] || b.estimatedCount - a.estimatedCount
      })
      .slice(0, SCENARIO_DB_ENEMY_ENTITY_IDS.length)
    const dbEnemies: ScenarioEntity[] = sortedEnemyRows.map((row, index) => {
      const id = SCENARIO_DB_ENEMY_ENTITY_IDS[index]!
      const base = normalizeEnemyIngressPoint(row.lat, row.lng)
      const pose = enemyBattlefieldPoses[id]
      return {
        id,
        name: row.codename,
        lat: pose?.lat ?? base.lat,
        lng: pose?.lng ?? base.lng,
        relation: 'ENEMY',
        kind: 'MBT',
        status: `${row.threatLevel} · 관측 ${row.observedAt}`,
        speedKph: 420,
        headingDeg: 180 + ((row.id % 5) - 2) * 7,
        riskLevel: row.threatLevel,
        enemyCategory: 'armored_battalion',
        detectionStatus: row.threatLevel === '높음' ? 'tracking' : row.threatLevel === '중간' ? 'identified' : 'detected',
        confidence: 'suspected',
        trackId: String(row.id),
        grdTankEstimate: row.estimatedCount,
        grdRiskScore: row.threatLevel === '높음' ? 92 : row.threatLevel === '중간' ? 74 : 58,
        echelonLevel: (['중대', '대대', '연대'] as const)[index % 3]!,
      }
    })
    if (dbEnemies.length > 0) {
      return grdCluster ? [...dbEnemies, grdCluster, ...nonEnemy] : [...dbEnemies, ...nonEnemy]
    }
    return DUMMY_SCENARIO_ENTITIES.map((e) => {
      const p = enemyBattlefieldPoses[e.id]
      return p ? { ...e, lat: p.lat, lng: p.lng } : e
    })
  }, [enemyBattlefieldPoses, enemyInfiltrationsDb])
  ...
  /**
   * GRD 노란색(전차) 폴리곤: 최초 적 MBT 위치에 스냅한 뒤 `frozenYellowGrdMbtCentersByMotionId`로 고정.
  ...
  const liveGrdMotion = useMemo(() => {
    ...
    const liveEnemies = scenarioEntitiesResolved
      .filter(
        (e) =>
          e.relation === 'ENEMY' &&
          e.kind === 'MBT' &&
          (e.enemyCategory === 'armored_battalion' || e.enemyCategory === 'grd_motion_cluster'),
      )
      .map((e) => ({ lat: e.lat, lng: e.lng }))
    const yellowSpec = GRD_DETECTION_SPEC.filter((s) => s.classLabel === '전차')
    const blueSpec = GRD_DETECTION_SPEC.filter((s) => s.classLabel !== '전차')
    ...
    > = {}
    yellowSpec.forEach((spec, i) => {
      const live = liveEnemies[i]
      const pinned = frozenYellowGrdMbtCentersByMotionId?.[spec.id]
      const cx = pinned ? pinned.lng : live ? live.lng : spec.cx
      const cy = pinned ? pinned.lat : live ? live.lat : spec.cy
      features.push({
        type: 'Feature',
        id: spec.id,
        properties: {
  ...
  useEffect(() => {
    if (frozenYellowGrdMbtCentersByMotionId != null) return
    const poses = enemyBattlefieldPoses
    if (Object.keys(poses).length === 0) return
    ...
    for (let i = 0; i < yellowSpec.length; i += 1) {
      const spec = yellowSpec[i]!
      const e = enemies[i]
      if (!e) continue
      const pose = poses[e.id]
      if (!pose) return
      next[spec.id] = { lat: pose.lat, lng: pose.lng }
    }
    if (Object.keys(next).length === 0) return
    setFrozenYellowGrdMbtCentersByMotionId(next)
  }, [enemyBattlefieldPoses, scenarioEntitiesResolved, frozenYellowGrdMbtCentersByMotionId])
  ...
  const [, setTacticScores] = useState<ScenarioTacticScore[] | null>(null)
  ...
  const [targetRosterModalOpen, setTargetRosterModalOpen] = useState(false)
  /** 표적 일람표 적 탭 — 전송용 다중 선택(침투 표적 id) */
  const [targetRosterSelectedEnemyIds, setTargetRosterSelectedEnemyIds] = useState<number[]>([])
  /** 표적 일람표(아군) 위치 옆 채팅 — 우측 도크 수신 부대 id */
  const [targetRosterChatUnitId, setTargetRosterChatUnitId] = useState<number | null>(null)
  /** 표적 일람표 — 선택 표적 전송 시 수신 아군 부대 */
  const [targetRosterSendReceiverUnitId, setTargetRosterSendReceiverUnitId] = useState<number | null>(null)
  const [opsRegionSearchInput, setOpsRegionSearchInput] = useState('')
  const [fmcwCoverageOn, setFmcwCoverageOn] = useState(false)
  const [fmcwPredictionRouteOn, setFmcwPredictionRouteOn] = useState(false)
  const [fmcwPredictionRiskOn, setFmcwPredictionRiskOn] = useState(false)
  ...
  const [dispatchModalReceiverPresetId, setDispatchModalReceiverPresetId] = useState<number | null>(null)
  const [dispatchModalSeed, setDispatchModalSeed] = useState(0)
  ...
  const [assetStreamUavBand, setAssetStreamUavBand] = useState<'eo' | 'ir'>('eo')
  ...
  const {
    messages: allDispatchMessages,
    filteredMessages: dispatchMessages,
    addMessage: addDispatchMessage,
    receiverFilter: dispatchLogReceiverFilter,
    setReceiverFilter: setDispatchLogReceiverFilter,
  } = useDispatchMessages()
  ...
  /** 모달 배경 닫기 직후 클릭이 지도로 전달되어 적/GRD 재처리되는 것을 막음 */
  const suppressMapSelectPassThroughUntilRef = useRef(0)
  ...
  const assetStreamCandidates = useMemo(() => {
    ...
    return ordered.map((asset, index) => {
      ...
      return {
        ...
        streamVideoUrlEo: resolvePlaybackMediaUrl(rawEo),
        streamVideoUrlIr: resolvePlaybackMediaUrl(rawIr),
      }
    })
  ...
  const selectedAssetStreamVideoUrl = useMemo(() => {
    if (!selectedAssetStream) return null
    if (assetStreamModal?.sensor !== 'uav') return selectedAssetStream.streamVideoUrl ?? null
    return assetStreamUavBand === 'ir'
      ? (selectedAssetStream.streamVideoUrlIr ?? selectedAssetStream.streamVideoUrlEo ?? null)
      : (selectedAssetStream.streamVideoUrlEo ?? selectedAssetStream.streamVideoUrlIr ?? null)
  }, [assetStreamModal, assetStreamUavBand, selectedAssetStream])
  ...
  const grdDispatchEligible = useMemo(() => {
    ...
    if (!(liveGrdMotion.meta[grdFocusId] ?? GRD_MOTION_META[grdFocusId])) return false
    return uavDispatchAssets.length > 0
  }, [grdFocusId, uavDispatchAssets, liveGrdMotion])
  ...
  const openUavDispatchModal = useCallback(
    (request: UavDispatchTarget) => {
      ...
      setEnemyDispatchPanelMode('dispatch')
      setSelectedEnemyTacticNames([])
      setUavDispatchGrdPanel(
        request.kind === 'grd' && request.grdModalEntry === 'sar_detection' ? 'sar' : 'dispatch',
      )
  ...
  const openUavDispatchModalRef = useRef(openUavDispatchModal)
  openUavDispatchModalRef.current = openUavDispatchModal
  ...
  useEffect(() => {
    ...
    if (simulationPaused || timelineApplyingRef.current) {
      ...
      const mbtList = scenarioEntitiesResolvedRef.current.filter((e) => e.relation === 'ENEMY' && e.kind === 'MBT')
  ...
  useEffect(() => {
    ...
    const loadAssets = (attempt: number) => {
      ...
      void Promise.all([
        requestJson<FriendlyUnit[]>(`${getApiBaseUrl()}/map/units`),
        requestJson<EnemyInfiltration[]>(`${getApiBaseUrl()}/map/infiltrations`).catch(() => []),
      ])
        .then(([units, infiltrations]) => {
          if (cancelled) return
          setFriendlyUnitsRaw(units)
          setEnemyInfiltrationsDb(Array.isArray(infiltrations) ? infiltrations : [])
          const next = units
            .map((unit) => {
  ...
  useEffect(() => {
    if (!targetRosterModalOpen) return
    let cancelled = false
    void requestJson<EnemyInfiltration[]>(`${getApiBaseUrl()}/map/infiltrations`)
      .then((rows) => {
        if (!cancelled && Array.isArray(rows)) {
          setEnemyInfiltrationsDb(rows)
        }
      })
      .catch(() => {
        /* 시드/백엔드 미기동 시 기존 목록 유지 */
      })
    return () => {
      cancelled = true
    }
  }, [targetRosterModalOpen])

  const grouped = useMemo(
    () => ({
      SAR: assets.filter((asset) => asset.category === 'SAR'),
      SATELLITE_SAR: assets.filter((asset) => asset.category === 'SATELLITE_SAR'),
  ...
  const friendlyUnitRows = useMemo((): FriendlyUnitTableRow[] => {
    const rawById = new Map<number, FriendlyUnit>()
    for (const unit of friendlyUnitsRaw) rawById.set(unit.id, unit)
    const nowIso = new Date().toISOString()
    const rows = assets
      .filter(
        (asset) =>
          asset.category !== 'SAR' &&
          asset.category !== 'SATELLITE_SAR' &&
          asset.category !== 'GROUND_RADAR',
      )
      .map((asset) => {
        const raw = rawById.get(asset.id)
        if (!unitUpdatedAtRef.current[asset.id]) unitUpdatedAtRef.current[asset.id] = nowIso
        return {
          id: asset.id,
          unitCode: asset.unitCode,
          name: asset.name,
          level: asset.level,
          branch: raw?.branch ?? SERVICE_CATEGORY_LABEL[asset.category],
          formation: asset.formation,
          lat: asset.lat,
          lng: asset.lng,
          locationText: asset.mgrs,
          mission: asset.mission,
          readiness: asset.readiness,
          equipment: raw?.equipment ?? '-',
          personnel: raw?.personnel ?? null,
          updatedAt: unitUpdatedAtRef.current[asset.id]!,
          note: raw?.locationStatus === 'PLANNED' ? '계획 위치' : '-',
          symbolLabel: raw ? TACTICAL_SYMBOL_LABEL[raw.symbolType] : SERVICE_CATEGORY_LABEL[asset.category],
          situationMediaUrl: asset.situationVideoUrl ?? raw?.situationVideoUrl ?? null,
        } satisfies FriendlyUnitTableRow
      })
    return rows
  }, [assets, friendlyUnitsRaw])

  const targetRosterSendReceiverOptions = useMemo(
    () =>
      friendlyUnitRows.map((u) => ({
        id: u.id,
        label: `${u.unitCode} · ${u.name}`,
      })),
    [friendlyUnitRows],
  )
  ...
  useEffect(() => {
    if (!targetRosterModalOpen) return
    setTargetRosterSendReceiverUnitId((prev) => {
      if (prev != null && friendlyUnitRows.some((u) => u.id === prev)) return prev
      return friendlyUnitRows.find((u) => isBattalionC2Unit(u))?.id ?? friendlyUnitRows[0]?.id ?? null
    })
  }, [targetRosterModalOpen, friendlyUnitRows])

  const targetRosterEnemyRows = useMemo((): TargetRosterTableRow[] => {
    const order: Record<EnemyInfiltration['threatLevel'], number> = { 높음: 0, 중간: 1, 낮음: 2 }
    const sorted = [...enemyInfiltrationsDb].sort(
      (a, b) =>
        order[a.threatLevel] - order[b.threatLevel] ||
        b.estimatedCount - a.estimatedCount ||
        a.id - b.id,
    )
    return sorted.map((e, i) => ({
      rank: i + 1,
      targetNo: `TGT-${String(e.id).padStart(5, '0')}`,
      enemyInfiltrationId: e.id,
      location: latLngToMgrsSafe(e.lat, e.lng),
      lat: e.lat,
      lng: e.lng,
      elevation:
        e.elevationM != null && Number.isFinite(e.elevationM)
          ? `${Math.round(e.elevationM)} m`
          : '—',
      scale: `${e.estimatedCount}명(추정)`,
      status: `${e.threatLevel} · 관측 ${e.observedAt}`,
      note: `${e.codename} · ${e.enemyBranch}`,
    }))
  }, [enemyInfiltrationsDb])

  const targetRosterFriendlyRows = useMemo((): TargetRosterTableRow[] => {
    const filtered = friendlyUnitsRaw.filter((u) => {
      const cat = branchToServiceCategory(u.branch)
      return cat != null && FRIENDLY_ROSTER_CATEGORY_SET.has(cat)
    })
    const sorted = [...filtered].sort((a, b) => a.id - b.id)
    return sorted.map((u, i) => ({
      rank: i + 1,
      targetNo:
        u.unitCode != null && String(u.unitCode).trim().length > 0
          ? String(u.unitCode).trim()
          : `A${String(u.id).padStart(5, '0')}`,
      location: u.mgrs != null && u.mgrs.trim().length > 0 ? u.mgrs.trim() : latLngToMgrsSafe(u.lat, u.lng),
      unitId: u.id,
      lat: u.lat,
      lng: u.lng,
      elevation: Number.isFinite(u.elevationM) ? `${(u.elevationM as number).toFixed(0)} m` : '—',
      scale: `${u.personnel}명 · ${u.level}`,
      status: u.readiness,
      note: [u.name, u.mission].filter((s) => s && s.length > 0).join(' · '),
    }))
  }, [friendlyUnitsRaw])

  const targetRosterAssetRows = useMemo((): TargetRosterTableRow[] => {
    const rawById = new Map<number, FriendlyUnit>()
    for (const unit of friendlyUnitsRaw) rawById.set(unit.id, unit)
    const orderIdx = (c: ServiceAssetCategory) => {
      const i = SERVICE_ASSET_ROSTER_ORDER.indexOf(c)
      return i === -1 ? 99 : i
    }
    const list = assets
      .filter((a) => SERVICE_ASSET_ROSTER_ORDER.includes(a.category))
      .sort((a, b) => {
        const d = orderIdx(a.category) - orderIdx(b.category)
        return d !== 0 ? d : a.id - b.id
      })
    return list.map((asset, i) => {
      const raw = rawById.get(asset.id)
      const catLabel =
        SERVICE_ASSET_STATUS_SIDEBAR_LABEL[asset.category] ?? SERVICE_CATEGORY_LABEL[asset.category]
      return {
        rank: i + 1,
        targetNo:
          asset.unitCode != null && String(asset.unitCode).trim().length > 0
            ? String(asset.unitCode).trim()
            : asset.name,
        location:
          asset.mgrs != null && String(asset.mgrs).trim().length > 0
            ? String(asset.mgrs).trim()
            : latLngToMgrsSafe(asset.lat, asset.lng),
        lat: asset.lat,
        lng: asset.lng,
        serviceAssetId: asset.id,
        elevation:
          raw != null && Number.isFinite(raw.elevationM) ? `${(raw.elevationM as number).toFixed(0)} m` : '—',
        scale: `${catLabel} · ${asset.level}`,
        status: raw?.readiness ?? asset.readiness,
        note:
          raw != null
            ? [raw.name, raw.mission].filter((s) => s && s.length > 0).join(' · ')
            : [asset.name, asset.mission].filter((s) => s && s.length > 0).join(' · '),
      }
    })
  }, [assets, friendlyUnitsRaw])

  const handleTargetRosterEnemyNavigate = useCallback((row: TargetRosterTableRow) => {
    const lat = row.lat
    const lng = row.lng
    if (lat == null || lng == null || !Number.isFinite(lat) || !Number.isFinite(lng)) return
    const map = mapRef.current
    if (!map) return
    setLayerVisible((prev) => (prev.enemy ? prev : { ...prev, enemy: true }))
    map.flyTo({
      center: [lng, lat],
  ...
  const handleTargetRosterAssetNavigate = useCallback((row: TargetRosterTableRow) => {
    const lat = row.lat
    const lng = row.lng
    if (lat == null || lng == null || !Number.isFinite(lat) || !Number.isFinite(lng)) return
    const map = mapRef.current
    if (!map) return
    setLayerVisible((prev) => (prev.friendly ? prev : { ...prev, friendly: true }))
    map.flyTo({
      center: [lng, lat],
  ...
  const handleApplyBattalionHqToUserGeolocation = useCallback(() => {
    ...
    setUserBattalionGeoLoading(true)
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
          ...
          setScenarioNotice('유효하지 않은 좌표를 받았습니다.')
          return
        }
        const mgrs = latLngToMgrsSafe(lat, lng)
        unitUpdatedAtRef.current[unit.id] = new Date().toISOString()
        setFriendlyUnitsRaw((prev) =>
          prev.map((u) => (u.id === unit.id ? { ...u, lat, lng, mgrs } : u)),
        )
        setAssets((prev) => {
          let changed = false
          const next = prev.map((asset) => {
            if (asset.id !== unit.id) return asset
            changed = true
            return { ...asset, lat, lng, mgrs }
          })
      ...
      (err: GeolocationPositionError) => {
        ...
        setScenarioNotice(
          denied ? '위치 권한이 거부되어 좌표를 가져올 수 없습니다.' : '현재 위치를 가져오지 못했습니다. 잠시 후 다시 시도하세요.',
        )
      },
  ...
  const handleOpsRegionSearchSubmit = useCallback(() => {
    const map = mapRef.current
    if (!map || !mapReady) {
      setScenarioNotice('지도가 준비된 뒤 검색할 수 있습니다.')
      return
    }
    const hit = resolveOpsRegionSearch(opsRegionSearchInput)
    if (!hit) {
      setScenarioNotice('일치하는 지역이 없습니다. 예: 서울, 부산, 한반도, 군사분계선(MDL), 평양')
      return
    }
    if (hit.kind === 'fitBounds') {
      map.fitBounds(
        [
          [hit.west, hit.south],
          [hit.east, hit.north],
        ],
        { padding: 52, duration: 780, maxZoom: hit.maxZoom ?? 9.5 },
      )
    } else {
      map.flyTo({
    ...
    setScenarioNotice(`작전 지역 이동: ${hit.label}`)
  }, [mapReady, opsRegionSearchInput])
  ...
  const assetsForBattlefieldMap = useMemo(() => {
    return assets.map((a) => {
      ...
      const pos = orbitalGroundPosition(slot, orbitalSimSeconds)
      return {
        ...a,
        lat: pos.lat,
        lng: pos.lng,
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      setOrbitalSimSeconds((s) => (s + 1) % ORBITAL_PERIOD_SEC)
    }, 1000)
  ...
  const sensorPaths = useMemo(() => {
    ...
    return {
      sar:
        grouped.SAR.length + grouped.SATELLITE_SAR.length > 0
          ? [...grouped.SAR, ...grouped.SATELLITE_SAR].map((asset) => ({ lat: asset.lat, lng: asset.lng }))
          : fallback,
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      for (const e of mbtList) {
        ...
        initialPoses[e.id] = snapEnemyMbtPoseToLandCorridor({ lat: e.lat, lng: e.lng })
        ...
        if (!goal) continue
        const from: MarchPoint = snapEnemyMbtPoseToLandCorridor({ lat: e.lat, lng: e.lng })
        ...
        if (cum.length > 0) {
          initialPoses[e.id] = snapEnemyMbtPoseToLandCorridor(positionAlongPolylineM(fallback, cum, 0))
        }
      }
      ...
      await Promise.all(
        mbtList.map(async (e) => {
          ...
          try {
            ...
            const snappedRoute = snapMarchPolylineVertices(res.coordinates)
            if (!isEnemyMarchLandPolyline(snappedRoute)) return
            if (cancelled) return

            const nextPoly = snappedRoute
      ...
      setEnemyBattlefieldPoses((prev) => {
        ...
        for (const e of mbtList) {
          ...
          if (poly && cum?.length) {
            ...
            next[e.id] = snapEnemyMbtPoseToLandCorridor(positionAlongPolylineM(poly, cum, m))
          }
        }
        return next
      })
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setEnemyBattlefieldPoses((prev) => {
        const next = { ...prev }
        for (const e of scenarioEntitiesResolvedRef.current) {
  ...
  useEffect(() => {
    ...
    for (const entity of scenarioEntitiesResolvedRef.current) {
  ...
  const dronePredictedRouteForMap = useMemo((): Parameters<GeoJSONSource['setData']>[0] => {
    ...
    for (const point of enemyPoints) {
      const distKm = haversineKm(droneSimPos, point)
      if (distKm <= BATTLEFIELD_DRONE_ENEMY_ACQUIRE_KM && (!nearest || distKm < nearest.distKm)) {
        nearest = { lat: point.lat, lng: point.lng, distKm }
      }
    }
    if (nearest) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      const attachMdlGeoJson = (data: Parameters<GeoJSONSource['setData']>[0]) => {
        if (map.getSource(SERVICE_MDL_LINE_SOURCE_ID)) return
        map.addSource(SERVICE_MDL_LINE_SOURCE_ID, {
          type: 'geojson',
          data,
        })
        if (!map.getLayer(SERVICE_MDL_LINE_LAYER_ID)) {
          map.addLayer({
            id: SERVICE_MDL_LINE_LAYER_ID,
            type: 'line',
            source: SERVICE_MDL_LINE_SOURCE_ID,
            paint: {
              'line-color': 'rgba(194, 65, 12, 0.95)',
              'line-width': 2,
              'line-opacity': 0.92,
      ...
      void fetch(MDL_OSM_GEOJSON_PUBLIC_PATH)
        .then((res) => {
          if (!res.ok) throw new Error(String(res.status))
          return res.json() as Promise<Parameters<GeoJSONSource['setData']>[0]>
        })
        .then((data) => attachMdlGeoJson(data))
        .catch(() => attachMdlGeoJson(DMZ_MDL_LATITUDE_FALLBACK_GEOJSON))
      ...
      if (!map.getSource(SERVICE_ORBITAL_SAR_TRACE_SOURCE_ID)) {
        map.addSource(SERVICE_ORBITAL_SAR_TRACE_SOURCE_ID, {
          type: 'geojson',
          data: buildOrbitalTracesGeoJson(),
        })
      }
      if (!map.getSource(SERVICE_ORBITAL_SAR_FOOTPRINT_SOURCE_ID)) {
        map.addSource(SERVICE_ORBITAL_SAR_FOOTPRINT_SOURCE_ID, {
          type: 'geojson',
          data: { type: 'FeatureCollection', features: [] },
        })
      }
      if (!map.getLayer(SERVICE_ORBITAL_SAR_TRACE_LAYER_ID)) {
        map.addLayer({
          id: SERVICE_ORBITAL_SAR_TRACE_LAYER_ID,
          type: 'line',
          source: SERVICE_ORBITAL_SAR_TRACE_SOURCE_ID,
          paint: {
            'line-color': [
              'match',
              ['get', 'slot'],
      ...
      if (!map.getLayer(SERVICE_ORBITAL_SAR_FOOTPRINT_FILL_LAYER_ID)) {
        map.addLayer({
          id: SERVICE_ORBITAL_SAR_FOOTPRINT_FILL_LAYER_ID,
          type: 'fill',
          source: SERVICE_ORBITAL_SAR_FOOTPRINT_SOURCE_ID,
          paint: {
      ...
      if (!map.getLayer(SERVICE_ORBITAL_SAR_FOOTPRINT_LINE_LAYER_ID)) {
        map.addLayer({
          id: SERVICE_ORBITAL_SAR_FOOTPRINT_LINE_LAYER_ID,
          type: 'line',
          source: SERVICE_ORBITAL_SAR_FOOTPRINT_SOURCE_ID,
          paint: {
            'line-color': [
        ...
        map.addLayer({
          ...
          paint: {
            ...
            'circle-color': [
              ...
              'SATELLITE_SAR',
              SERVICE_CATEGORY_COLOR.SATELLITE_SAR,
              'UAV',
        ...
        map.addLayer({
          ...
          layout: {
            'icon-image': [
              ...
              'SATELLITE_SAR',
              SERVICE_ASSET_SYMBOL_IMAGE_ID.SATELLITE_SAR,
              'UAV',
            ...
            'icon-size': SERVICE_ASSETS_SYMBOL_ICON_SIZE_LAYOUT,
      ...
      if (map.getLayer(SERVICE_ASSETS_SYMBOL_LAYER_ID)) {
        map.setLayoutProperty(
          SERVICE_ASSETS_SYMBOL_LAYER_ID,
          'icon-size',
          SERVICE_ASSETS_SYMBOL_ICON_SIZE_LAYOUT,
        )
        map.setLayoutProperty(
          SERVICE_ASSETS_SYMBOL_LAYER_ID,
          'icon-offset',
      ...
      if (!map.getLayer(SERVICE_ASSETS_ECHELON_LAYER_ID)) {
        map.addLayer({
          id: SERVICE_ASSETS_ECHELON_LAYER_ID,
          type: 'symbol',
          source: SERVICE_ASSETS_SYMBOL_SOURCE_ID,
          filter: [
            'all',
            ['!', ['has', 'point_count']],
            ['>', ['length', ['coalesce', ['get', 'echelonRankMark'], '']], 0],
          ],
          layout: {
      ...
      /** SAR-2 관측 구역: 시나리오(적 심볼·라벨)보다 아래에 두어 클릭·호버 우선순위가 표적 쪽으로 가도록 함 */
      ...
      if (!map.getLayer(SERVICE_SAR2_ZONE_FILL_LAYER_ID)) {
        map.addLayer({
          ...
          /** 광역(ScanSAR)은 면이 아니라 점선만 표시 — 내부 호버는 막고 스팟라이트만 면 픽킹 */
          filter: ['==', ['get', 'id'], 'sar2-spotlight-zone'],
          paint: {
            'fill-color': '#22c55e',
            'fill-opacity': 0.12,
          },
        ...
        map.addSource(SERVICE_SCENARIO_SOURCE_ID, {
          ...
          data: toMapSourceData(
            scenarioEntitiesResolvedRef.current
              .filter((entity) => scenarioMbtEnemyVisibleOnMap(entity, {}))
      ...
      if (!map.getLayer(SERVICE_SCENARIO_ENEMY_ECHELON_LAYER_ID)) {
        map.addLayer({
          id: SERVICE_SCENARIO_ENEMY_ECHELON_LAYER_ID,
          type: 'symbol',
          source: SERVICE_SCENARIO_SOURCE_ID,
          filter: [
            'all',
          ...
          paint: {
            'text-color': '#78350f',
            'text-halo-color': 'rgba(254, 243, 199, 0.95)',
      ...
      map.on('mouseenter', SERVICE_ASSETS_ECHELON_LAYER_ID, () => {
        map.getCanvas().style.cursor = 'pointer'
      })
      ...
      map.on('mouseleave', SERVICE_ASSETS_ECHELON_LAYER_ID, () => {
        map.getCanvas().style.cursor = ''
        if (popupRef.current) {
          popupRef.current.remove()
          popupRef.current = null
      ...
      map.on('mousemove', SERVICE_ASSETS_ECHELON_LAYER_ID, handleAssetLayerMouseMove)
      ...
      const handleAssetLayerClick = (event: MapLayerMouseEvent) => {
        ...
        const isSatelliteSar = category === 'SATELLITE_SAR' || isOrbitalSarAssetId(idNum)
        if (
        ...
        if (isSatelliteSar) {
          map.fitBounds(
            ...
            { padding: 32, duration: 900, maxZoom: 2.15 },
          )
        }
        setSelectedDetail({
          ...
          ...(isSatelliteSar
            ? {
                orbitalSarAssetId: idNum,
                orbitAltitudeKm: ORBITAL_LEO_ALTITUDE_KM,
                orbitalGroundSpeedKmh: ORBITAL_GROUND_SPEED_KMH,
              }
            : { elevationM }),
        })
        if (category === 'UAV') {
      ...
      map.on('click', SERVICE_ASSETS_ECHELON_LAYER_ID, handleAssetLayerClick)
      ...
      const handleEnemyScenarioMouseMove = (event: MapLayerMouseEvent) => {
        ...
        const targetMeta = scenarioEntitiesResolvedRef.current.find((entity) => entity.id === idNum)
      ...
      const handleEnemyScenarioClick = (event: MapLayerMouseEvent) => {
        ...
        if (Date.now() < suppressMapSelectPassThroughUntilRef.current) return

        const deferDroneUntilDispatchModalConfirm =
          ph === BattlefieldScenarioPhase.SAR_SCAN && target.kind === 'MBT'

        if (!deferDroneUntilDispatchModalConfirm) {
          launchDroneFromEnemyClick({
            id: target.id,
            kind: target.kind,
            name: formatScenarioEnemyCompact(target),
      ...
      popupActionClickHandler = (event: MouseEvent) => {
        ...
        const targetMeta = scenarioEntitiesResolvedRef.current.find((entity) => entity.id === entityId)
        if (!targetMeta) return
      ...
      map.on('click', SERVICE_ALLY_LAYER_ID, (event) => {
        ...
        if (!feature) return
        const idNum = Number((feature.properties ?? {}).id)
        const target = scenarioEntitiesResolvedRef.current.find((entity) => entity.id === idNum)
        if (!target) return
        setSelectedDetail({
          title: target.name,
          affiliation: '우군',
      ...
      /** ScanSAR 광역(sar2-wide-zone): 점선 테두리에만 호버 시 툴팁 — 면(fill)은 레이어에서 제외됨 */
      map.on('mouseenter', SERVICE_SAR2_ZONE_LINE_LAYER_ID, () => {
        map.getCanvas().style.cursor = 'pointer'
      })
      map.on('mousemove', SERVICE_SAR2_ZONE_LINE_LAYER_ID, (event) => {
        const wideHit = event.features?.find(
          (f) => String((f.properties as Record<string, unknown> | undefined)?.id ?? '') === 'sar2-wide-zone',
        )
        const spotHit = event.features?.find(
          (f) => String((f.properties as Record<string, unknown> | undefined)?.id ?? '') === 'sar2-spotlight-zone',
        )
        const zoneFeature = wideHit ?? spotHit
        ...
        const html = renderSarZoneObservationTooltipHtml({
          zoneName: String(zoneProps.name ?? 'SAR-2 관측 지역'),
          zoneNote: zoneProps.note != null ? String(zoneProps.note) : undefined,
          route: routeProps,
        })
        if (!popupRef.current) {
      ...
      map.on('mouseleave', SERVICE_SAR2_ZONE_LINE_LAYER_ID, () => {
        map.getCanvas().style.cursor = ''
        if (popupRef.current) {
          popupRef.current.remove()
          popupRef.current = null
      ...
      map.on('click', SERVICE_SAR2_ZONE_FILL_LAYER_ID, (event) => {
        ...
        // 요청 반영: 추가 광역 탐지 영역(sar2-wide-zone)은 클릭 이벤트 비활성화
        if (zoneId === 'sar2-wide-zone') return
        ...
        setSelectedDetail({
          ...
          lat: 39.7017,
          lng: 127.315,
          mgrs: latLngToMgrsSafe(39.7017, 127.315),
      ...
      map.on('click', (event) => {
        if (Date.now() < suppressMapSelectPassThroughUntilRef.current) return
        const enemyPickLayerIds = scenarioEnemyPickLayers.filter((id) => map.getLayer(id))
        const enemyHitsAtPoint =
          enemyPickLayerIds.length > 0
            ? map.queryRenderedFeatures(event.point, { layers: enemyPickLayerIds })
            : []
        if (map.getLayer(SERVICE_GRD_MOTION_FILL_LAYER_ID) && enemyHitsAtPoint.length === 0) {
          ...
          if (grdHits.length > 0) {
            ...
            if (motionIdRaw != null) {
              ...
              if (phaseAtLeast(scenarioPhaseRef.current, BattlefieldScenarioPhase.SAR_SCAN)) {
                const meta = liveGrdMotionMetaRef.current[motionIdStr] ?? GRD_MOTION_META[motionIdStr]
                if (meta) {
                  ...
                  const blueVehicleGrd = isGrdBlueVehicleClass(meta.classLabel)
                  if (!blueVehicleGrd) {
                  ...
                  if (scenarioPhaseRef.current === BattlefieldScenarioPhase.SAR_SCAN) {
                    openUavDispatchModalRef.current({
                      kind: 'grd',
                      title: grdDispatchTitleByMotionId(motionIdStr),
                      summary: grdDispatchSummaryByMeta(motionIdStr, meta),
                      lat: meta.centerLat,
                      lng: meta.centerLng,
                      motionId: motionIdStr,
                      ...(blueVehicleGrd ? { grdModalEntry: 'sar_detection' } : {}),
                    })
                  }
                }
              }
      ...
      // 첫 진입은 기본 세계 시야가 아니라 "작전 구역 선택"과 동일한 한반도 시야로 시작
      map.fitBounds(
        [
          [KOREA_OPS_BOUNDS.west, KOREA_OPS_BOUNDS.south],
          [KOREA_OPS_BOUNDS.east, KOREA_OPS_BOUNDS.north],
        ],
        { padding: 60, duration: 0, maxZoom: 7.4 },
      )
  ...
  useEffect(() => {
    ...
    const source = map.getSource(SERVICE_ORBITAL_SAR_FOOTPRINT_SOURCE_ID)
    if (!source || !('setData' in source)) return
    const positions = ORBITAL_SAR_IDS.map((id) => {
      const slot = ORBITAL_SLOT_BY_ID[id]!
      const pos = orbitalGroundPosition(slot, orbitalSimSeconds)
      return { id, lat: pos.lat, lng: pos.lng }
    })
    ;(source as GeoJSONSource).setData(buildOrbitalFootprintsGeoJson(positions, orbitalSarModes))
  }, [mapReady, orbitalSimSeconds, orbitalSarModes])
  ...
  useEffect(() => {
    ...
    applyVisibility(SERVICE_ASSETS_ECHELON_LAYER_ID, layerVisible.friendly)
    ...
    applyVisibility(SERVICE_SCENARIO_ENEMY_ECHELON_LAYER_ID, enemySymbolOn)
  ...
  /** GRD(이동 검출) 폴리곤을 live 적 MBT 좌표에 스냅 */
  useEffect(() => {
    if (!mapReady) return
    const map = mapRef.current
    if (!map) return
    const src = map.getSource(SERVICE_GRD_MOTION_SOURCE_ID) as
      | { setData?: (d: unknown) => void }
      | undefined
    if (src?.setData) {
      try {
        src.setData(liveGrdMotion.geojson)
      } catch {
        /* noop */
      }
    }
  }, [mapReady, liveGrdMotion])
  ...
  useEffect(() => {
    ...
    const onMove = (e: MapLayerMouseEvent) => {
      ...
      const meta = liveGrdMotionMetaRef.current[motionId] ?? GRD_MOTION_META[motionId]
      if (!meta) return
    ...
    (sensorId: ServiceSensorId) => {
      ...
      if (advancedTo === BattlefieldScenarioPhase.UAV_DISPATCHED) {
        ...
        if (dispatchTarget) {
          ...
          const fid = g.focusId
          const liveMetaLookup = liveGrdMotionMetaRef.current
          if (fid && (liveMetaLookup[fid] ?? GRD_MOTION_META[fid])) {
            const m = liveMetaLookup[fid] ?? GRD_MOTION_META[fid]!
            uavGrdMissionRef.current = { centerLat: m.centerLat, centerLng: m.centerLng }
            setActiveUavDispatchTarget({
              kind: 'grd',
              title: grdDispatchTitleByMotionId(fid),
              summary: grdDispatchSummaryByMeta(fid, m),
              lat: m.centerLat,
              lng: m.centerLng,
              motionId: fid,
            })
  ...
  const selectedContextLabel = useMemo(() => {
    ...
    return `${selectedDetail.affiliation} · ${selectedDetail.title}`
  }, [selectedDetail])

  const selectedUnitRecentMessages = useMemo(() => {
    if (selectedAssetId == null || !selectedDetail || selectedDetail.affiliation !== '아군') return []
    return allDispatchMessages.filter((row) => row.receiverUnitId === selectedAssetId).slice(0, 3)
  }, [allDispatchMessages, selectedAssetId, selectedDetail])

  const handleSendDispatchMessage = useCallback(
    (payload: {
      ...
      const attachmentSummary = [
        payload.attachments.enemyPosition ? '적 위치' : null,
        payload.attachments.riskZones ? '위험지역' : null,
        payload.attachments.detections ? '탐지 결과' : null,
        payload.attachments.predictedRoute ? '이동 경로 예측' : null,
        payload.attachments.sensorAnalysis ? '센서 분석 결과' : null,
      ]
      ...
      addDispatchMessage({
        ...
        receiverUnitCode: receiver.unitCode,
        ...
        relatedRiskZoneId: payload.attachments.riskZones ? (selectedRiskCandidateId ?? null) : null,
        relatedInferenceResultId:
          payload.attachments.sensorAnalysis
            ? `fmcw-${groundRadarVodAnalytics.detectedCount}-${groundRadarVodAnalytics.clusterCount}`
            : null,
        createdAt: new Date().toISOString(),
        readAt: null,
      })
      setScenarioNotice(`정보 전파 완료: ${receiver.unitCode} ${receiver.name} 대상으로 전송했습니다.`)
    },
    [
      addDispatchMessage,
      friendlyUnitRows,
      groundRadarVodAnalytics.clusterCount,
      groundRadarVodAnalytics.detectedCount,
      selectedDetail?.affiliation,
      selectedRiskCandidateId,
    ],
  )
  ...
  const openDispatchModal = useCallback(
    (receiverUnitId?: number) => {
      setDispatchModalReceiverPresetId(receiverUnitId ?? selectedAssetId ?? null)
      setDispatchModalSeed((prev) => prev + 1)
  ...
  const closeDispatchModal = useCallback(() => {
    ...
    setDispatchModalReceiverPresetId(null)
  }, [])

  const handleSendDispatchMessageInModal = useCallback(
    (payload: {
      receiverUnitId: number
      title: string
      content: string
      ...
      handleSendDispatchMessage(payload)
      closeDispatchModal()
    },
    [closeDispatchModal, handleSendDispatchMessage],
  )

  const targetRosterChatUnit = useMemo(
    () =>
      targetRosterChatUnitId == null
        ? null
        : (friendlyUnitRows.find((u) => u.id === targetRosterChatUnitId) ?? null),
    [friendlyUnitRows, targetRosterChatUnitId],
  )

  const targetRosterChatMessages = useMemo(() => {
    if (targetRosterChatUnitId == null) return []
    return allDispatchMessages.filter((m) => m.receiverUnitId === targetRosterChatUnitId)
  }, [allDispatchMessages, targetRosterChatUnitId])

  useEffect(() => {
    if (targetRosterChatUnitId == null) return
    const exists = friendlyUnitRows.some((u) => u.id === targetRosterChatUnitId)
    if (!exists) setTargetRosterChatUnitId(null)
  }, [friendlyUnitRows, targetRosterChatUnitId])

  const handleTargetRosterFriendlyChatOpen = useCallback((row: TargetRosterTableRow) => {
    if (row.unitId == null || !Number.isFinite(row.unitId)) return
    setTargetRosterChatUnitId(row.unitId)
  }, [])

  const handleTargetRosterChatSend = useCallback(
    (content: string) => {
      if (targetRosterChatUnitId == null) return
  ...
  const handleTargetRosterToggleSelectAllEnemies = useCallback(() => {
    const allIds = targetRosterEnemyRows
  ...
  }, [targetRosterEnemyRows])

  const handleTargetRosterSendSelectedEnemies = useCallback(() => {
    if (targetRosterSelectedEnemyIds.length === 0) return
    if (targetRosterSendReceiverUnitId == null) {
      setScenarioNotice('수신 아군 부대를 선택하세요.')
      return
    }
    const receiver = friendlyUnitRows.find((u) => u.id === targetRosterSendReceiverUnitId)
    if (!receiver) {
      setScenarioNotice('선택한 수신 부대를 찾을 수 없습니다.')
      return
    }
    ...
    setTargetRosterSelectedEnemyIds([])
  }, [
    friendlyUnitRows,
    handleSendDispatchMessage,
    targetRosterEnemyRows,
    targetRosterSelectedEnemyIds,
    targetRosterSendReceiverUnitId,
  ])

  const closeTargetRosterModal = useCallback(() => {
    setTargetRosterModalOpen(false)
    setTargetRosterSelectedEnemyIds([])
  ...
  const handleSensorSimulationModalProceed = useCallback(() => {
    ...
    if (sensorSimModalSensor === 'uav' && scenarioPhase === BattlefieldScenarioPhase.SAR_SCAN) {
      const liveFocusMeta = grdFocusId
        ? liveGrdMotionMetaRef.current[grdFocusId] ?? GRD_MOTION_META[grdFocusId]
        : undefined
      if (grdFocusId && grdDispatchEligible && liveFocusMeta) {
        const meta = liveFocusMeta
        openUavDispatchModal({
          kind: 'grd',
          title: grdDispatchTitleByMotionId(grdFocusId),
          summary: grdDispatchSummaryByMeta(grdFocusId, meta),
          lat: meta.centerLat,
          lng: meta.centerLng,
          motionId: grdFocusId,
          ...(isGrdBlueVehicleClass(meta.classLabel) ? { grdModalEntry: 'sar_detection' } : {}),
        })
        setSensorSimModalSensor(null)
        return
      }
  ...
  const focusAssetOnMap = useCallback((asset: ServiceAssetPoint) => {
    ...
    if (asset.category === 'SATELLITE_SAR') {
      map.fitBounds(
        [
          [-175, -58],
          [185, 70],
  ...
  const handleCategoryClick = useCallback(
    (category: ServiceAssetCategory) => {
      ...
      const map = mapRef.current
      if (!map) return
      if (category === 'SATELLITE_SAR') {
        map.fitBounds(
          [
            [-175, -58],
            [185, 70],
          ],
          { padding: 72, duration: 650, maxZoom: 2.2 },
        )
        return
      }
  ...
  const handleAssetClick = useCallback(
    (asset: ServiceAssetPoint) => {
      if (
        scenarioPhase === BattlefieldScenarioPhase.REGION_SELECTED && asset.category === 'SAR'
      ) {
        setScenarioNotice(
  ...
  const handleResetScenario = useCallback(() => {
    ...
    setFmcwCoverageOn(false)
    setFmcwPredictionRouteOn(false)
    setFmcwPredictionRiskOn(false)
  ...
  }, [applySensorSimulationStep, handleResetScenario, scenarioPhase])
  ...
  const timelineControlEnabled =
    primaryScenarioCta.mode === 'speed' && phaseAtLeast(scenarioPhase, BattlefieldScenarioPhase.SAR_SCAN)
  ...
  const handleToggleSimulationPause = useCallback(() => {
    ...
    setSimulationPaused((prev) => {
      const next = !prev
      // 재개 시 SAR Spotlight를 닫지 않아도 행군·타임라인이 진행되도록(팝업은 그대로 둘 수 있음)
      if (!next) {
        setSarSpotlightSeen(true)
      }
  ...
  const handlePlaybackTimelineChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    ...
    setEnemyBattlefieldPoses((prev) => {
      ...
      for (const entity of scenarioEntitiesResolvedRef.current) {
        ...
        const pos = snapEnemyMbtPoseToLandCorridor(positionAlongPolylineM(poly, cum, targetM))
        next[entity.id] = pos
        changed = true
      }
      return changed ? next : prev
    ...
    <section className="page service-battlefield-page">
      ...
      <AppShell splitClassName={`service-map-layout${droneSplitViewActive ? ' service-map-layout--drone-split' : ''}`}>
        ...
        <div className="service-map-shell">
          ...
          <div
            className="service-ops-region-search"
            role="search"
            aria-label="작전 지역 검색"
            onClick={(event) => event.stopPropagation()}
          >
            <form
              className="service-ops-region-search__form"
              onSubmit={(event) => {
                event.preventDefault()
                handleOpsRegionSearchSubmit()
              }}
            >
              <label htmlFor={opsRegionSearchInputId} className="service-ops-region-search__label">
                작전 지역 검색
              </label>
              <div className="service-ops-region-search__row">
                <input
                  ...
                  value={opsRegionSearchInput}
                  onChange={(event) => setOpsRegionSearchInput(event.target.value)}
                  placeholder="지역명 입력 후 검색"
                  autoComplete="off"
                  enterKeyHint="search"
                />
                <button type="submit" className="service-ops-region-search__btn" disabled={!mapReady}>
                  검색
                </button>
              </div>
            </form>
            ...
            >
              ...
              <dl className="service-detail-dl">
                  ...
                  <div>
                    <dt>표고</dt>
                    <dd>
                      {selectedDetail.orbitAltitudeKm != null
                        ? `LEO 궤도고 약 ${selectedDetail.orbitAltitudeKm} km`
                        : `${battlefieldPanelElevationM(selectedDetail)} m`}
                    </dd>
                  </div>
                  {selectedDetail.orbitalGroundSpeedKmh != null && (
                    <div>
                      <dt>지상 투영 속도</dt>
                      <dd>약 {(selectedDetail.orbitalGroundSpeedKmh / 1000).toFixed(0)}k km/h</dd>
                    </div>
                  )}
                  {selectedDetail.orbitalSarAssetId != null && (
                    <div className="service-orbital-sar-detail">
                      <div>
                        <dt>관측 모드</dt>
                        <dd>
                          <div className="service-orbital-sar-mode-row" role="group" aria-label="SAR 관측 모드">
                            <button
                              type="button"
                              className={
                ...
                {selectedUnitRecentMessages.length > 0 && (
                  <div className="dispatch-panel__selected-unit-log">
                    <h4>최근 수신 전파</h4>
                    <ul>
          ...
          {targetRosterModalOpen &&
            createPortal(
              <TargetRosterModal
                open={targetRosterModalOpen}
                onClose={closeTargetRosterModal}
                loading={assetLoading}
                loadError={assetError}
                enemyRows={targetRosterEnemyRows}
                friendlyRows={targetRosterFriendlyRows}
                assetRows={targetRosterAssetRows}
                selectedEnemyInfiltrationIds={targetRosterSelectedEnemyIds}
                onToggleEnemySelected={handleTargetRosterToggleEnemySelected}
                onToggleSelectAllEnemies={handleTargetRosterToggleSelectAllEnemies}
                onSendSelectedEnemies={handleTargetRosterSendSelectedEnemies}
                sendReceiverOptions={targetRosterSendReceiverOptions}
                selectedSendReceiverUnitId={targetRosterSendReceiverUnitId}
                onChangeSendReceiverUnitId={setTargetRosterSendReceiverUnitId}
                onEnemyRowNavigate={handleTargetRosterEnemyNavigate}
                onFriendlyLocationChat={handleTargetRosterFriendlyChatOpen}
                onAssetRowNavigate={handleTargetRosterAssetNavigate}
              />,
              document.body,
            )}
          {targetRosterChatUnitId != null &&
            targetRosterChatUnit &&
            createPortal(
              <TargetRosterChatDock
                unitId={targetRosterChatUnit.id}
                unitCode={targetRosterChatUnit.unitCode}
                unitName={targetRosterChatUnit.name}
          ...
          {dispatchModalOpen &&
            createPortal(
              <div className="dispatch-modal-backdrop" role="presentation" onClick={closeDispatchModal}>
                <div className="dispatch-modal-sheet" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
                  <DispatchPanel
                    key={`dispatch-modal-${dispatchModalSeed}`}
                    mode="modal"
                    onClose={closeDispatchModal}
                    units={friendlyUnitRows}
                    selectedUnitId={dispatchModalReceiverPresetId ?? selectedAssetId}
                    selectedContextLabel={selectedContextLabel}
                    onSend={handleSendDispatchMessageInModal}
                    logs={dispatchMessages}
                    logReceiverFilter={dispatchLogReceiverFilter}
                    onChangeLogReceiverFilter={setDispatchLogReceiverFilter}
                  />
                </div>
              </div>,
              document.body,
            ...
            createPortal(
              <div
                ...
                onPointerDown={(event) => {
                  ...
                  suppressMapSelectPassThroughUntilRef.current = Date.now() + 450
                }}
                onClick={(event) => {
                  event.stopPropagation()
                  closeUavDispatchModal()
              ...
              >
                <div
                  className={`service-uav-dispatch-modal${uavDispatchRequest.kind === 'enemy' ? ' service-uav-dispatch-modal--enemy' : ''}${
                    uavDispatchRequest.kind === 'grd' && uavDispatchGrdPanel === 'sar'
                      ? ' service-uav-dispatch-modal--grd-sar'
                      : ''
                  }`}
                  onClick={(event) => event.stopPropagation()}
                >
                <div className="service-uav-dispatch-modal__head">
                  ...
                  <div className="service-uav-dispatch-modal__head-actions">
                    {uavDispatchRequest.kind === 'grd' &&
                      uavDispatchRequest.grdModalEntry === 'sar_detection' &&
                      uavDispatchGrdPanel === 'dispatch' && (
                        <button
                          type="button"
                          className="service-uav-dispatch-modal__head-tab-back"
                          onClick={() => setUavDispatchGrdPanel('sar')}
                        >
                          탐지 영상
                        </button>
                      )}
                    ...
                    <>
                      ...
                      <div className="service-uav-dispatch-modal__actions">
                        ...
                        <button
                          type="button"
                          className="btn-primary"
                          onClick={() => setUavDispatchGrdPanel('dispatch')}
                        >
                          UAV 출동
                        </button>
                      </div>
                ...
                >
                  ...
                  <div className="service-asset-stream-modal__body">
                    ...
                    <div className="service-asset-stream-modal__player">
                      {assetStreamModal.sensor === 'uav' && (
                        <div className="service-asset-stream-modal__band-tabs" role="tablist" aria-label="UAV 영상 채널">
                          <button
                            type="button"
                            role="tab"
                            aria-selected={assetStreamUavBand === 'eo'}
                            className={`service-asset-stream-modal__band-tab${assetStreamUavBand === 'eo' ? ' service-asset-stream-modal__band-tab--active' : ''}`}
                            onClick={() => setAssetStreamUavBand('eo')}
                          >
                            EO 영상
                          </button>
                          <button
                            type="button"
                            role="tab"
                            aria-selected={assetStreamUavBand === 'ir'}
                            className={`service-asset-stream-modal__band-tab${assetStreamUavBand === 'ir' ? ' service-asset-stream-modal__band-tab--active' : ''}`}
                            onClick={() => setAssetStreamUavBand('ir')}
                          >
                            IR 영상
                          </button>
                        </div>
                      )}
                      {selectedAssetStreamVideoUrl ? (
                        <video
                          key={`${selectedAssetStream?.id ?? 'asset'}-${assetStreamUavBand}-${selectedAssetStreamVideoUrl}`}
                          className="map-video-modal-video"
                          src={selectedAssetStreamVideoUrl}
                          controls
                          autoPlay
                          muted
                          loop
        ...
        <ScenarioSidebar ref={battlefieldSidebarRef} hidden={droneSplitViewActive}>
          <section className="service-panel-section">
            <div className="service-sim-control">
              ...
              {grdEnemyMotionAlert && (
                <div
                  className="service-sar-move-notice service-sar-move-notice--warning"
                  role="button"
                  tabIndex={0}
                ...
                >
                  <div className="service-sar-move-notice__head">
                    ...
                    <span className="service-sar-move-notice__badge service-sar-move-notice__badge--warn">
                      긴급
                    </span>
                  </div>
          ...
          <section className="service-panel-section">
            ...
            <ul className="service-asset-summary">
              {SERVICE_ASSET_STATUS_CATEGORIES.map((category) => {
                const headLabel =
                  SERVICE_ASSET_STATUS_SIDEBAR_LABEL[category] ?? SERVICE_CATEGORY_LABEL[category]
                return (
                          ...
                          return (
                            <li key={asset.id} data-service-sidebar-asset-id={asset.id}>
                              {isInlineReturnCategory ? (
                                ...
                                >
                                  <div className="service-asset-item-drone-row">
                                    ...
                                    >
                                      ...
                                      {isActiveDrone && (
                                        <span className="service-asset-item-btn__status">사용중 · 표적 추적 중</span>
                                      )}
                                    </button>
                                    <button
                                      type="button"
                                    ...
                                    <button
                                      type="button"
                                      className="btn-secondary service-asset-item-return-btn service-asset-item-return-btn--inline"
                                      onClick={() => openDispatchModal(asset.id)}
                                    >
                                      전파
                                    </button>
                                  </div>
                                ...
                                <div className="service-asset-item-inline-actions">
                                  <button
                                    type="button"
          ...
          <section className="service-panel-section">
            ...
            <ul className="service-asset-summary">
              {SERVICE_FRIENDLY_FORCE_CATEGORIES.map((category) => {
                const headLabel =
                  SERVICE_FRIENDLY_FORCE_SIDEBAR_LABEL[category] ?? SERVICE_CATEGORY_LABEL[category]
                return (
                  <li key={category} className="service-asset-group">
                    <button
                      type="button"
...
function AppLayout({ user, onLogout }: AppLayoutProps) {
  ...
  const [unitSymbolManualOpen, setUnitSymbolManualOpen] = useState(false)
  ...
  useEffect(() => {
    if (!unitSymbolManualOpen) return
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setUnitSymbolManualOpen(false)
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [unitSymbolManualOpen])
  ...
  return (
    <div className={`app-shell${sidebarCollapsed ? ' app-shell--sidebar-collapsed' : ''}`}>
      ...
      {unitSymbolManualOpen &&
        createPortal(
          <UnitSymbolManualModal onClose={() => setUnitSymbolManualOpen(false)} />,
          document.body,
        )}
    </div>
  )
...
/**
 * 단대호(표준 부대 부호) 매뉴얼 팝업 — 프레임 상단에 찍히는 에셜론 부호 표.
 * 참조: 표준 군대 부호 규격(단대호).
 */
type UnitSymbolManualRow = {
  level: string
  mark: string
  note: string
  kind: 'dot' | 'bar' | 'cross' | 'special'
}

const UNIT_SYMBOL_MANUAL_ROWS: ReadonlyArray<UnitSymbolManualRow> = [
  { level: '소대', mark: '•', note: '점 1개', kind: 'dot' },
  { level: '중대', mark: '|', note: '수직선 1개', kind: 'bar' },
  { level: '대대', mark: '||', note: '수직선 2개', kind: 'bar' },
  { level: '연대', mark: '|||', note: '수직선 3개', kind: 'bar' },
  { level: '사단', mark: '××', note: '×(가위표) 2개', kind: 'cross' },
  { level: '군단', mark: '×××', note: '×(가위표) 3개', kind: 'cross' },
  { level: '특수임무부대(전투단)', mark: '◆', note: '특수 기호(마름모)', kind: 'special' },
]

function UnitSymbolManualModal({ onClose }: { onClose: () => void }) {
  return (
    <div
      className="map-video-modal-backdrop"
      role="dialog"
      aria-modal="true"
      aria-labelledby="unit-symbol-manual-title"
      onClick={onClose}
    >
      <div
        className="map-video-modal unit-symbol-manual-modal"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="unit-symbol-manual-modal__header">
          ...
          <button
            type="button"
            className="unit-symbol-manual-modal__close"
            onClick={onClose}
            aria-label="닫기"
          >
        ...
        <div className="unit-symbol-manual-body">
          <table className="unit-symbol-manual-table">
            <thead>
              <tr>
                <th scope="col">부대급</th>
                <th scope="col">부호</th>
            ...
            <tbody>
              {UNIT_SYMBOL_MANUAL_ROWS.map((row) => (
                <tr key={row.level}>
                  <th scope="row">{row.level}</th>
                  <td>
                    <span
                      className={`unit-symbol-manual-mark unit-symbol-manual-mark--${row.kind}`}
                      aria-label={`${row.level} 부호`}
                    >
                      {row.mark}
                    </span>
                  </td>
...
function SensorTheoryPage() {
  const [theoryTab, setTheoryTab] = useState<'satellite_sar' | 'uav' | 'ground_radar' | 'drone'>(
    'satellite_sar',
  )
  const [satSarTheoryStepIndex, setSatSarTheoryStepIndex] = useState(0)

  const satelliteSarTheorySteps = [
    {
      title: '원본 SAR 영상',
      imageUrl: '/media/sar/sat-sar-step-1.png',
      caption: '처리 전 원본 강도 영상으로 배경 구조와 관심 구역을 파악합니다.',
    },
    {
      title: 'Azimuth 에너지 프로파일',
      imageUrl: '/media/sar/sat-sar-step-2.png',
      caption: '행 방향 에너지 곡선에서 burst 구간을 탐지해 분석 대상을 분리합니다.',
    },
    {
      title: '이동 표적 후보 추출',
      imageUrl: '/media/sar/sat-sar-step-3.png',
      caption: '배경 대비 변화가 큰 픽셀을 추출해 이동 후보군을 구성합니다.',
    },
    {
      title: 'DBSCAN 군집화',
      imageUrl: '/media/sar/sat-sar-step-4.png',
      caption: '후보 점 군집화로 노이즈를 줄이고 표적 집합 윤곽을 계산합니다.',
    },
    {
      title: '주 피크(본체) 검출',
      imageUrl: '/media/sar/sat-sar-step-5.png',
      caption: '군집 내부에서 주요 피크를 골라 실제 이동체 본체를 선별합니다.',
    },
    {
    ...
    },
    {
      ...
      imageUrl: '/media/sar/sat-sar-step-7.png',
      caption: '확정 표적만 남긴 최종 뷰로 전술 인계 전 결과를 검토합니다.',
    },
  ] as const

  const theoryContentByTab: Record<
    'satellite_sar' | 'uav' | 'ground_radar' | 'drone',
    { title: string; summary: string; details: string[] }
  > = {
    satellite_sar: {
      title: '위성 SAR',
      summary: '광역 감시로 초기 이상 징후를 빠르게 포착하고 관심 구역을 좁힙니다.',
      details: [
        '장점: 광역 커버리지와 재방문 기반 변화 탐지',
        '역할: 1차 후보 AOI 추출 후 UAV/드론으로 인계',
        '한계: 세부 식별은 근접 센서가 보완',
      ],
    },
    uav: {
      title: 'UAV',
      summary: '표적 인근으로 접근해 EO/IR로 확인하고 추적 우선순위를 정합니다.',
      details: [
        '장점: 위성 대비 높은 공간 해상도',
        '역할: 의심 표적 확인 및 위협도 평가',
        '운용: 상황별 경로/체공 시간 조정으로 연속 감시',
      ],
    },
    ground_radar: {
      title: '지상감시 레이더',
      summary: '전술 권역 이동 표적을 연속 추적해 경로·속도 변화를 제공합니다.',
      details: [
        '장점: 야간·악천후에서도 안정적 탐지',
        '역할: 탐지/추적 데이터로 위험 구역 계산',
        '연계: FMCW/지상 센서 결과를 C2에서 통합 표출',
      ],
    },
    drone: {
      title: '드론',
      summary: '최종 근접 식별 단계에서 표적 유형을 확정하고 영상을 공유합니다.',
      details: [
        '장점: 저고도 근접 관측으로 오탐 제거',
        '역할: 최종 식별·실시간 영상 전파',
        '운용: 드론별 고정 영상 매핑으로 일관된 분석',
      ],
    },
  }
  const activeTheory = theoryContentByTab[theoryTab]
  const activeSatelliteSarTheoryStep = satelliteSarTheorySteps[satSarTheoryStepIndex]!

  return (
    <section className="page theory-page">
      <div className="service-panel-section theory-page__shell">
        <h1 className="theory-page__title">센서 이론 설명</h1>
        <div className="theory-page__tabs" role="tablist" aria-label="센서 이론 탭">
          <button
            type="button"
            role="tab"
            aria-selected={theoryTab === 'satellite_sar'}
            className={`sidebar-theory-tab${theoryTab === 'satellite_sar' ? ' sidebar-theory-tab--active' : ''}`}
            onClick={() => {
              setTheoryTab('satellite_sar')
              setSatSarTheoryStepIndex(0)
            }}
          >
            위성 SAR
          </button>
          <button
            type="button"
            role="tab"
            aria-selected={theoryTab === 'uav'}
            className={`sidebar-theory-tab${theoryTab === 'uav' ? ' sidebar-theory-tab--active' : ''}`}
            onClick={() => setTheoryTab('uav')}
          >
            UAV
          </button>
          <button
            type="button"
            role="tab"
            aria-selected={theoryTab === 'ground_radar'}
            className={`sidebar-theory-tab${theoryTab === 'ground_radar' ? ' sidebar-theory-tab--active' : ''}`}
            onClick={() => setTheoryTab('ground_radar')}
          >
            지상감시 레이더
          </button>
          <button
            type="button"
            role="tab"
            aria-selected={theoryTab === 'drone'}
            className={`sidebar-theory-tab${theoryTab === 'drone' ? ' sidebar-theory-tab--active' : ''}`}
            onClick={() => setTheoryTab('drone')}
          >
            드론
          </button>
        </div>
        <section className="sidebar-theory-panel">
          <h3>{activeTheory.title}</h3>
          <p className="muted">{activeTheory.summary}</p>
          <ul>
            {activeTheory.details.map((line) => (
              <li key={line}>{line}</li>
            ))}
          </ul>
          {theoryTab === 'satellite_sar' && (
            <div className="sidebar-theory-step-viewer">
              <div className="sidebar-theory-step-viewer__head">
                <strong>{activeSatelliteSarTheoryStep.title}</strong>
                <span className="muted">
                  {satSarTheoryStepIndex + 1} / {satelliteSarTheorySteps.length}
                </span>
              </div>
              <img
                src={activeSatelliteSarTheoryStep.imageUrl}
                alt={`위성 SAR 처리 단계 ${satSarTheoryStepIndex + 1}`}
                className="sidebar-theory-step-viewer__image"
              />
              <p className="muted sidebar-theory-step-viewer__caption">
                {activeSatelliteSarTheoryStep.caption}
              </p>
              <div className="sidebar-theory-step-viewer__actions">
                <button
                  type="button"
                  className="btn-secondary"
                  onClick={() => setSatSarTheoryStepIndex((prev) => Math.max(0, prev - 1))}
                  disabled={satSarTheoryStepIndex <= 0}
                >
                  이전
                </button>
                <button
                  ...
                  className="btn-primary"
                  onClick={() =>
                    setSatSarTheoryStepIndex((prev) =>
                      Math.min(satelliteSarTheorySteps.length - 1, prev + 1),
                    )
                  }
                  disabled={satSarTheoryStepIndex >= satelliteSarTheorySteps.length - 1}
                >
                  다음
                </button>
              </div>
...
function App() {
  ...
  const handleLogout = () => {
    /**
     * BattlefieldServicePage는 지도 · 다수 interval · video 등 무거운 리소스를 쥐고 있어
     * user=null로 React 내에서 동기 unmount하면 cleanup이 연쇄 실행되며 화면이 수 초간 멈춘다.
     * 로그아웃은 세션을 버리는 동작이므로 하드 네비게이션으로 전 SPA를 재부팅해 끊김을 없앤다.
     */
    try {
      localStorage.removeItem('accessToken')
    } catch {
    ...
    <Routes>
      <Route element={<RequireAuth user={user} authReady={authReady} />}>
        <Route element={<AppLayout user={user} onLogout={handleLogout} />}>
          ...
          <Route path="/theory" element={<SensorTheoryPage />} />
