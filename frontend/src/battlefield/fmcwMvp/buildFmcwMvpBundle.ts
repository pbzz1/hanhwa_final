...
function strikeAssessment(
  ...
  if (intelOnly) {
    return {
      strikeCapable: false,
      rationale: '정찰·감시 자산 — 직접 화력 배제(더미 규칙)',
    }
  }
  if (category === 'ARTILLERY') {
    if (distKm <= 42) {
      return {
        strikeCapable: true,
        rationale: inZone
          ? '포병 사거리 내 · 위험구역 경계 근접(더미)'
          : '포병 사거리 내 간접 사격 가능(더미)',
      }
    }
    return { strikeCapable: false, rationale: '사거리 한계 추정 초과(더미)' }
  }
  if (category === 'ARMOR') {
    if (distKm <= 28) {
      return {
    ...
    return { strikeCapable: false, rationale: '기갑 전개 거리 밖(더미)' }
  }
  if (category === 'DIVISION' || category === 'UPPER_COMMAND') {
    ...
    return { strikeCapable: false, rationale: '작전 깊이 밖(더미)' }
  }
  return { strikeCapable: false, rationale: '유형 미분류(더미)' }
}
...
export function buildFmcwMvpBundle(assets: AssetLike[]): FmcwMvpBundle {
  ...
  return {
    zoneLabel: '함흥 남쪽 · FMCW 위험 윤곽(더미)',
    detectionRangeKm: FMCW_DETECTION_RANGE_KM,
    approachSpeedMps: FMCW_APPROACH_SPEED_MPS,
    ingressSummary: `남서→북동 축, 대략 ${ingressBearingDeg}° 방위로 위험구역 진입 예측(더미)`,
    ingressBearingDeg,
    ...
    radarReportLines: [
      ...
      `활성 트랙 ${FMCW_MOCK_TRACKS.length}개 — BEV·트랙 리스트는 mock`,
    ],
  }
}
